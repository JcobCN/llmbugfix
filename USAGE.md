...
- 非 dry-run 模式只允许推送 `ai/*` 分支，并要求 `origin` 主机在 allow-list 中；推送目标是自己账号（`GITLAB_ACCOUNT`，默认 `codigger-llm`）在 `GITLAB_URL` GitLab 上的同名公开镜像仓库，仓库不存在时自动创建，已有私有仓库也会调整为公开。PAT 优先取自 `GITLAB_TOKEN`，并只在进程内缓存；没有 PAT 时可用 `GITLAB_PASSWORD` 在当前进程中生成，Git 的网络操作通过主机限定的内存 credential helper 使用它，不读写 `~/.git-credentials`；推送也不会创建 MR 或合并。
...
| `GITLAB_URL` | `http://172.29.100.126` | 自己账号公开镜像仓库所在的 GitLab 地址（见 `docs/gitlab-private-repo-api.md`）；设为空字符串则回退推送到 origin |
| `GITLAB_ACCOUNT` | `codigger-llm` | 公开镜像仓库所属账号 |
| `GITLAB_TOKEN` | 未设置 | GitLab PAT；进程内使用，不读写 `~/.git-credentials` |
| `GITLAB_PASSWORD` | 未设置 | `GITLAB_TOKEN` 未设置时，可显式配置以在当前进程通过 Web 登录生成 PAT；源码没有默认密码 |
...
`dev_env_snapshot` 必填；`dev_env_special` 可填可不填。如果传入，必须是最多 100 个非空字符串组成的数组；空数组等同于不填写。每项代表一个 module 的特殊版本，例如 `raw-spofer-pel v2.0.200`。
...
| push 被拒绝 | 保持 `DRY_RUN=true` 做验证；确认当前分支是 `ai/*`、origin 存在且主机在 `GIT_ALLOWED_HOSTS` 与 `RepoManager.allowedRemoteHosts` 中；非 dry-run 推送目标是 `GITLAB_URL`（默认 `http://172.29.100.126`）上 `GITLAB_ACCOUNT` 账号的同名公开镜像仓库（不存在时自动创建），检查 `GITLAB_TOKEN` 是否有效，或确认 `GITLAB_PASSWORD` 能完成当前进程的 PAT bootstrap |
