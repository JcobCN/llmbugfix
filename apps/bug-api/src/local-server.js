...
if (llmEnabled) {
    ...
    environments = {
        ...
        provisionProfile: async (proposal) => {
            ...
            if (!repositoryUrl)
                throw new Error('Git repository remote address is required');
...
if (llmEnabled && !sandboxProfile)
    console.warn('WARNING: real Pi repair worker disabled; set PI_SANDBOX_PROFILE to an externally enforced sandbox profile before enabling Pi bash.');
let closing = false;
