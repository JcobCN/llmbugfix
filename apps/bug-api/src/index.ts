...
export class BugApiServer {
  ...
  private async handle(request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (path === '/api/bugs/conversations' || path === '/api/conversations') {
        ...
        const taskType = body.taskType === 'development' ? 'development' : 'bugfix'; const initialDraft: BugReportDraft = { taskType };
        const completeness = evaluateCompleteness(initialDraft);
        const conversation = this.repo.createConversation({ reporterId: user.id, status: 'active', draft: initialDraft, completeness });
        if (this.documentStore) this.documentStore.create(conversation.id, mergeBugDocument('', initialDraft, completeness));
        this.repo.appendMessage({ conversationId: conversation.id, role: 'assistant', content: taskType === 'development' ? '请描述希望新增或改造的能力、具体需求，以及完成后如何验收。' : '请直接描述你遇到的问题，包括做了什么、实际发生什么，以及期望结果。', metadata: {} });
  ...
  private async submit(conversation: BugConversation, request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    if (!title) { send(response, 422, { error: draft.taskType === 'development' ? '无法根据项目/模块和开发目标生成有效标题' : '无法根据项目/模块和问题现象生成有效标题', code: 'INVALID_TASK_TITLE' }); return; }
    ...
    const report = BugReportSchema.parse({ id: newId(), bugKey: `BUG-${Date.now()}`, taskType: draft.taskType ?? 'bugfix', title, productArea: draft.productArea ?? null, component: draft.component ?? null, bugType: draft.bugType ?? 'unknown', executionTarget: draft.executionTarget ?? 'unknown', environmentProfileId: draft.environmentProfileId ?? null, severity: draft.severity ?? 'unknown', actualBehavior: draft.actualBehavior ?? '', expectedBehavior: draft.expectedBehavior ?? null, objective: draft.objective ?? null, requirements: draft.requirements ?? [], acceptanceCriteria: draft.acceptanceCriteria ?? [], nonGoals: draft.nonGoals ?? [], constraints: draft.constraints ?? [], referenceContext: draft.referenceContext ?? [], reproduction: { reproducible: draft.reproduction?.reproducible ?? null, frequency: draft.reproduction?.frequency ?? 'unknown', prerequisites: draft.reproduction?.prerequisites ?? [], steps: draft.reproduction?.steps ?? [], testData: draft.reproduction?.testData ?? [] }, environment, evidence, impact, regression, observations: draft.observations ?? [], reporterHypotheses: draft.reporterHypotheses ?? [], reporter: { userId: user.id, displayName: user.displayName }, intake: { completenessScore: finalCompleteness.score, confidence: 0.5, missingInformation: finalCompleteness.missingCriticalInformation, conversationId: conversation.id, llmSummary: draft.taskType === 'development' ? draft.objective ?? '' : draft.actualBehavior ?? '' }, createdAt: now(), updatedAt: now() });
