...
export class BugApiServer {
  ...
  private async handle(request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (path === '/api/bugs/conversations' || path === '/api/conversations') {
        ...
        send(response, 201, conversationResponse(this.repo, conversation)); return;
      }
  ...
  private async optionalBody(request: http.IncomingMessage): Promise<Record<string, unknown>> { if (request.method !== 'POST' && request.method !== 'PATCH') return {}; return jsonBody(request); }
  private async handleConversation(id: string, action: string | undefined, method: string, request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    if (!action && method === 'GET') { send(response, 200, conversationResponse(this.repo, conversation)); return; }
    if (action === 'messages' && method === 'POST') {
      ...
      const processed = await this.intake.processUserMessage(conversation.draft, this.repo.listMessages(id), content, [...(this.manualFields.get(id) ?? new Set<string>()), ...(Array.isArray(body.userEditedFields) ? body.userEditedFields.map(String) : [])]);
      ...
      send(response, 200, { ...conversationResponse(this.repo, updated), turn: processed.turn }); return;
    }
    if (action === 'draft' && method === 'GET') { send(response, 200, { draft: conversation.draft, completeness: conversation.completeness }); return; }
    if (action === 'draft' && (method === 'PATCH' || method === 'POST')) {
      ...
      const updated = updateConversation(this.repo, id, draft, evaluateCompleteness(draft)); send(response, 200, conversationResponse(this.repo, updated)); return;
    }
  ...
  private async submit(conversation: BugConversation, request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    if (existing) { send(response, 200, { bug: existing, bugKey: existing.bugKey, idempotent: true }); return; }
    ...
    const draft = mergeDraft(conversation.draft, (body.draft ?? {}) as BugReportDraft);
    ...
    updateConversation(this.repo, conversation.id, draft, completeness, 'submitted'); send(response, 201, { bug: final, bugKey: final.bugKey, status: finalStatus, job, completeness });
  }
