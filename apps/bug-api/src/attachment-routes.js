import { AttachmentError } from '@llmbugfix/attachment-service';
...
export function createAttachmentRoutes(deps) {
    return async (request) => {
        ...
        if (upload) {
            if (method === 'GET')
                return deps.repo ? response(200, { attachments: deps.repo.listAttachments(upload[1]) }) : response(404, { error: 'Attachment listing is unavailable' });
            if (method !== 'POST')
                return response(405, { error: 'Method not allowed' });
            if (deps.repo && !deps.repo.getBug(upload[1]))
                return response(404, { error: 'Bug report not found' });
            ...
            if (!filename || !mimeType || !encoded)
                return response(400, { error: 'filename, mimeType and base64 data are required' });
            try {
                const buffer = Buffer.from(encoded, 'base64');
                // Reject malformed base64 rather than silently decoding arbitrary text.
                if (!buffer.length || encoded.replace(/\s/g, '').replace(/=+$/, '').length % 4 === 1)
                    return response(400, { error: 'Invalid base64 attachment data' });
                const attachment = deps.attachments.saveAttachment(upload[1], filename, mimeType, buffer);
                if (deps.repo)
                    deps.repo.addAttachment(upload[1], attachment);
                return response(201, { attachment });
            }
            catch (error) {
                return response(error instanceof AttachmentError ? 400 : 500, { error: error instanceof Error ? error.message : 'Attachment upload failed' });
            }
        }
        ...
        if (vision) {
            ...
            if (!relativePath)
                return response(400, { error: 'relativePath is required' });
            try {
                ...
                return response(200, { analysisStatus: 'failed', description: FALLBACK_VISION_PROMPT, visibleText: [], machineObservations: [], reporterObservations: [], confidence: 0, fallbackPrompt: FALLBACK_VISION_PROMPT });
            }
        }
        return undefined;
    };
