...
describe('attachment route plugin', () => {
    it('uploads base64 content and keeps vision degradation successful', async () => {
        ...
        const vision = await routes({ method: 'POST', pathname: '/internal/vision/analyze', body: { relativePath: (upload?.body).attachment.relativePath } });
        expect(vision?.status).toBe(200);
        expect((vision?.body).analysisStatus).toBe('unsupported');
        expect((vision?.body).reporterObservations).toEqual([]);
    });
...
//# sourceMappingURL=attachment-routes.test.js.map