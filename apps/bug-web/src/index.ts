...
const intakeClientScript = String.raw`(() => {
  ...
  function render(value) {
    ...
    $('score').textContent = completeness.score ?? 0;
  ...
  async function send() {
    ...
    try {
      await flushDocument();
      const updated = await api('/api/bugs/conversations/' + id + '/messages', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
