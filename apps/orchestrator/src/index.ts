...
import { BugFixTaskSchema, CodingTaskSchema, AgentFixResultSchema, AgentTaskResultSchema, FixCandidateMetadataSchema, ReviewResultSchema, GitResultSchema, type AttachmentRef, type CodingTask, type BugReport, type FixCandidateMetadata } from '@llmbugfix/bug-domain';
...
export class Orchestrator {
  ...
  private taskFor(bug: BugReport, profile: EnvironmentProfile, attachments = this.attachmentsFor(bug)): CodingTask { const common = { bugKey: bug.bugKey, title: bug.title, executionTarget: profile.target, environmentProfileId: profile.id, environment: bug.environment.additionalInfo, attachments, reporterObservations: bug.observations, machineObservations: [], missingInformation: bug.intake.missingInformation, completenessScore: bug.intake.completenessScore }; return CodingTaskSchema.parse(bug.taskType === 'development' ? { ...common, taskType: 'development', objective: bug.objective, requirements: bug.requirements, acceptanceCriteria: bug.acceptanceCriteria, nonGoals: bug.nonGoals, constraints: bug.constraints, referenceContext: bug.referenceContext } : { ...common, taskType: 'bugfix', actualBehavior: bug.actualBehavior, expectedBehavior: bug.expectedBehavior, reproductionSteps: bug.reproduction.steps, prerequisites: bug.reproduction.prerequisites, errorMessages: bug.evidence.errorMessages, stackTraces: bug.evidence.stackTraces, lastKnownGoodVersion: bug.regression.lastKnownGoodVersion, failingVersion: bug.regression.suspectedVersion, reporterHypotheses: bug.reporterHypotheses }); }
  ...
  private async processJob(jobId: string, bugId: string, workerId: string): Promise<void> {
    ...
    try {
      ...
      this.writeArtifact(artifactDir, 'task.json', bug); this.writeArtifact(artifactDir, 'coding-task.json', task);
      ...
      let fixResult: ReturnType<typeof AgentFixResultSchema.parse> | ReturnType<typeof AgentTaskResultSchema.parse>;
      if (candidateMetadata) {
        ...
        const result = task.taskType === 'development' ? await this.agentRunner.runCoder!(runnerInput) : await this.agentRunner.runFixer({ ...runnerInput, task: BugFixTaskSchema.parse(task) });
        fixerInFlight = false;
        this.checkpoint(jobId, bugId); fixResult = task.taskType === 'development' ? AgentTaskResultSchema.parse(result) : AgentFixResultSchema.strict().parse(result); this.writeArtifact(artifactDir, 'agent-result.json', fixResult);
        ...
        fixResult = task.taskType === 'development' ? AgentTaskResultSchema.parse({ ...fixResult, filesChanged: actualFiles }) : AgentFixResultSchema.strict().parse({ ...fixResult, filesChanged: actualFiles }); this.writeArtifact(artifactDir, 'agent-result.json', fixResult);
      }
      const completed = task.taskType === 'development' ? fixResult.status === 'completed' : fixResult.status === 'fixed'; if (!completed) { await this.transition(bug, 'FIX_FAILED', fixResult); this.queue.failJob(jobId, fixResult.blockedReason ?? 'Coding agent failed', workerId); pipeline.status = 'FIX_FAILED'; pipeline.error = fixResult.blockedReason; return; }
      ...
      const acceptance = review.acceptanceCriteriaMet ?? []; const addressed = task.taskType === 'development' ? review.taskAddressed === true && acceptance.length === task.acceptanceCriteria.length && acceptance.every((item) => item.met) : review.bugAddressed === true; if (review.verdict !== 'approve' || !addressed || review.regressionRisk === 'high') { await this.transition(bug, 'REVIEW_REJECTED', review); this.queue.failJob(jobId, `Review gate rejected: ${review.summary}`, workerId); pipeline.status = 'REVIEW_REJECTED'; pipeline.error = review.summary; return; }
