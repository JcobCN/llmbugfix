...
describe('JobQueue', () => {
  ...
  it('enforces an exclusive process lock and releases it', () => { const db = setup(); const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'queue-lock-')); const first = new JobQueue({ database: db } as never, dir); first.acquireProcessLock(); const second = new JobQueue({ database: db } as never, dir, { lockAcquireTimeoutMs: 0 }); expect(() => second.acquireProcessLock()).toThrow(/process lock/); first.close(); expect(() => second.acquireProcessLock()).not.toThrow(); second.close(); });
  it('takes over an empty or corrupt lock file left by a crashed holder', () => { const db = setup(); const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'queue-lock-')); fs.writeFileSync(path.join(dir, 'orchestrator.lock'), ''); const queue = new JobQueue({ database: db } as never, dir, { lockAcquireTimeoutMs: 0 }); expect(() => queue.acquireProcessLock()).not.toThrow(); queue.close(); });
  it('takes over a lock file whose holder pid no longer exists', () => { const db = setup(); const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'queue-lock-')); fs.writeFileSync(path.join(dir, 'orchestrator.lock'), JSON.stringify({ pid: 999_999_999 })); const queue = new JobQueue({ database: db } as never, dir, { lockAcquireTimeoutMs: 0 }); expect(() => queue.acquireProcessLock()).not.toThrow(); queue.close(); });
});
