...
describe('EnvironmentResolver', () => {
  it('loads ordered, separate markdown and skill context', () => { const value = fixture(); const result = new EnvironmentResolver(value.config, value.root).resolveProfile('frontend'); expect(result.profile.repository).toBe('${REPO}'); expect(result.context.map((entry) => entry.kind)).toEqual(['markdown', 'skill']); expect(result.docsContent).toBe('facts'); expect(result.skillContent).toBe('behavior'); });
  it('blocks missing, escaping and oversized files', () => {
    ...
    expect(() => new EnvironmentResolver(value.config, value.root).resolveProfile('frontend')).toThrowError(EnvironmentBlockedError);
    ...
    expect(() => new EnvironmentResolver(value.config, value.root, { maxFileBytes: 2 }).resolveProfile('frontend')).toThrowError(/size limit/);
  });
  it('blocks zero and ambiguous target matches while honoring explicit id', () => {
    ...
    const resolver = new EnvironmentResolver(value.config, value.root); expect(() => resolver.resolveProfile('mobile')).toThrowError(/BLOCKED|No environment/); expect(() => resolver.resolveProfile('frontend')).not.toThrow(); expect(() => resolver.resolveProfile('frontend', 'none')).toThrowError(/not found/);
  });
});
