...
export interface EnvironmentResolverOptions { maxFileBytes?: number; }
...
export class EnvironmentResolver {
  ...
  constructor(private readonly configPath: string, rootDir = process.cwd(), options: EnvironmentResolverOptions = {}) {
    this.rootDir = fs.realpathSync.native(path.resolve(rootDir)); this.maxFileBytes = options.maxFileBytes ?? 256 * 1024; this.reload();
  }
  public reload(): EnvironmentProfile[] {
    ...
    for (const profile of profiles.environments) { if (ids.has(profile.id)) throw new EnvironmentBlockedError(`Duplicate environment profile id: ${profile.id}`); ids.add(profile.id); }
    this.profiles = profiles.environments; return this.listProfiles();
  }
  ...
  public resolveProfile(target: string, requestedProfileId?: string): ResolvedEnvironment {
    ...
    if (requestedProfileId) { profile = this.profiles.find((candidate) => candidate.id === requestedProfileId); if (!profile) throw new EnvironmentBlockedError(`Requested environment profile "${requestedProfileId}" not found`, { target, requestedProfileId }); }
