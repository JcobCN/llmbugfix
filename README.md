...
Weekly email is disabled when `WEEKLY_EMAIL_USERNAME`, `WEEKLY_EMAIL_PASSWORD`, and `WEEKLY_EMAIL_RECIPIENTS` are all absent. To enable it, configure all three; recipients are comma-separated. `WEEKLY_EMAIL_SMTP_URL` may be omitted because it defaults to `smtps://mail.onecloud.cn:465`. If supplied, that exact URL is the only accepted value: port 465 uses implicit TLS and never a plaintext or STARTTLS downgrade. Certificate and hostname verification are enabled by default; `WEEKLY_EMAIL_ALLOW_INSECURE_TLS=true` is an explicit internal-test-only override for a self-signed or hostname-mismatched SMTP certificate and is not suitable for production.
...
Delivery diagnostics stored in SQLite and emitted to logs are single-line, bounded, and redact credentials and email addresses. They intentionally omit the SMTP server's raw response and recipient list. For troubleshooting, check that all three enabling variables are present and nonblank, that the sender is a valid mailbox, that recipient entries contain no empty item, and that outbound TCP 465 and the host CA trust store are available. If the internal-only override is enabled, startup emits a warning and the SMTP connection is vulnerable to server impersonation; use it only on an isolated network. Never paste the password, raw SMTP transcript, Bug logs, stacks, diffs, or attachments into an issue.

Automated tests use fake mail transports and never contact `mail.onecloud.cn`. A real delivery check must be an explicit human operation in an approved environment: back up the data directory, set the four `WEEKLY_EMAIL_*` values in the process environment (using the fixed URL), optionally set `WEEKLY_EMAIL_ALLOW_INSECURE_TLS=true` only for the isolated internal test case, start the service, confirm only the intended test recipients are configured, and verify both the plain-text and HTML alternatives in their inbox. Stop the service afterward and remove the password from shell history/environment according to local secret-handling policy. Do not add real credentials to `.env.example`, tests, artifacts, or source control.

## Local operation and dashboard

Copy `.env.example` to `.env` and keep `DRY_RUN=true` for initial validation. A static environment catalog is optional: when a report names a project that is not already configured, the real Intake asks for an HTTPS or SSH Git clone URL. After the tester explicitly confirms the report, the service clones it into `DATA_ROOT/repositories` and writes a runnable profile to `DATA_ROOT/generated-environments.yaml`; no `E2E_FRONTEND_REPOSITORY`, `FRONTEND_MAIN_REPOSITORY`, or `BACKEND_MAIN_REPOSITORY` setting is required for this flow. `LLM_BACKENDS_CONFIG_PATH` optionally points to a version-1 YAML Backend Registry that can declare separate heterogeneous OpenAI-compatible endpoints for Intake, Pi Fixer, and Pi Reviewer, including capabilities, quality tiers, weights, and `maxConcurrency`; keys are referenced with `apiKeyEnv` and are never stored in the repository. If it is unset, `LLM_ENDPOINT_URL` + `LLM_MODEL` create the capacity-one `legacy` backend. Dispatcher routing filters all requested dimensions, waits for capacity, and only fails over transport/server failures. The repair worker additionally requires the externally enforced `PI_SANDBOX_PROFILE`. `VISION_HOST` remains disabled unless explicitly integrated.

The primary integration surface is the asynchronous external REST API: `POST /api/v1/tasks` requires an `Idempotency-Key` and returns `202` with a task resource; clients poll `GET /api/v1/tasks/{taskId}`, its cursor-based `/events`, and `/result`. The complete OpenAPI 3.1 document is available at `GET /openapi.json`. `DISPATCHER_MAX_CONCURRENT_JOBS` defaults to `1`; `JOB_LEASE_TIMEOUT_MS` and `JOB_HEARTBEAT_INTERVAL_MS` control queue ownership and fencing. Authentication and rate limiting are intentionally out of scope for this release.
