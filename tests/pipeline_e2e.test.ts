...
`
  ...
  it('runs complete dry-run pipeline and writes all 9 artifacts', async () => {
    ...
    expect(fixTask).toMatchObject({ dev_env_snapshot: 'r35.1', dev_env_special: ['raw-spofer-pel v2.0.200', 'another-module v1.2.3'] });
  ...
  it('runs a development task through coder, validation and acceptance review', async () => {
    ...
    const artifactDir = path.join(dataRootDir, 'agent-results', bug.bugKey); expect(JSON.parse(fs.readFileSync(path.join(artifactDir, 'coding-task.json'), 'utf8'))).toMatchObject({ taskType: 'development', acceptanceCriteria: ['Export downloads a CSV with filtered rows'], dev_env_snapshot: 'r35.1', dev_env_special: ['raw-spofer-pel v2.0.200', 'another-module v1.2.3'] }); expect(JSON.parse(fs.readFileSync(path.join(artifactDir, 'agent-result.json'), 'utf8'))).toMatchObject({ taskType: 'development', status: 'completed' });
  });
