...
class FakeCommandRunner {
    calls = [];
    async run(command, args, _options) {
...
function getBugStatus(repo, bugIdOrKey) {
    const row = repo.database.prepare('SELECT status FROM bug_reports WHERE id = ? OR bug_key = ?').get(bugIdOrKey, bugIdOrKey);
...
describe('Orchestrator End-to-End Pipeline', () => {
    ...
    beforeEach(() => {
        ...
        fs.writeFileSync(envConfigFile, `
environments:
  - id: frontend-test
    name: Frontend Test
    target: frontend
...
`);
    ...
    it('runs complete dry-run pipeline and writes all 9 artifacts', async () => {
        ...
        const validator = new Validator(fakeRunner);
        const orchestrator = new Orchestrator({ DATA_ROOT: dataRootDir }, repo, queue, envResolver, repoManager, envRunner, fakeAgent, validator, { dryRun: true, artifactRoot: path.join(dataRootDir, 'agent-results') });
    ...
    it('runs push-enabled pipeline and reaches READY_FOR_HUMAN_REVIEW', async () => {
        ...
        const fakeAgent = new FakePiRunner();
        const validator = new Validator(fakeRunner);
        const orchestrator = new Orchestrator({ DATA_ROOT: dataRootDir }, repo, queue, envResolver, repoManager, envRunner, fakeAgent, validator, { dryRun: false, artifactRoot: path.join(dataRootDir, 'agent-results') });
        const handled = await orchestrator.tick();
        expect(handled).toBe(true);
    ...
    it('handles validation failure correctly', async () => {
        ...
        const fakeAgent = new FakePiRunner();
        const orchestrator = new Orchestrator({ DATA_ROOT: dataRootDir }, repo, queue, envResolver, repoManager, envRunner, fakeAgent, failingValidator, { dryRun: true, artifactRoot: path.join(dataRootDir, 'agent-results') });
        const handled = await orchestrator.tick();
        expect(handled).toBe(true);
    ...
    it('handles review rejection correctly', async () => {
        ...
        const validator = new Validator(fakeRunner);
        const orchestrator = new Orchestrator({ DATA_ROOT: dataRootDir }, repo, queue, envResolver, repoManager, envRunner, rejectingAgent, validator, { dryRun: true, artifactRoot: path.join(dataRootDir, 'agent-results') });
        const handled = await orchestrator.tick();
        expect(handled).toBe(true);
        expect(getBugStatus(repo, bug.id)).toBe('REVIEW_REJECTED');
    });
});
//# sourceMappingURL=pipeline_e2e.test.js.map